...
/**
 * libc facade. Scalar entry points (all-primitive args/returns) delegate to
 * {@link CGen.Posix} (generated by tools/cfngen.py), which binds them through
 * the Native Image C interface.
 * Functions that marshal pointers, strings or structs stay on the Panama FFM
 * path below. The public surface is unchanged so callers and the
 * {@code MockedStatic<Libc>} unit tests keep working.
 */
public final class Libc {
    ...
    // --- scalar entry points: Native Image C interface (see generated CGen) ---

    public static int unshare(int flags) {
        return CGen.Posix.unshare(flags);
    }

    public static int setns(int fd, int nstype) {
        return CGen.Posix.setns(fd, nstype);
    }

    public static int kill(int pid, int signal) {
        return CGen.Posix.kill(pid, signal);
    }

    public static int prctl(int op, long a, long b, long c, long d) {
        return CGen.Posix.prctl(op, a, b, c, d);
    }

    public static int umask(int mask) {
        return CGen.Posix.umask(mask);
    }

    public static int getpid() {
        return CGen.Posix.getpid();
    }

    public static int getppid() {
        return CGen.Posix.getppid();
    }

    public static int clearenv() {
        return CGen.Posix.clearenv();
    }

    public static int geteuid() {
        return CGen.Posix.geteuid();
    }

    public static int getegid() {
        return CGen.Posix.getegid();
    }

    public static int setresuid(int ruid, int euid, int suid) {
        return CGen.Posix.setresuid(ruid, euid, suid);
    }

    public static int setresgid(int rgid, int egid, int sgid) {
        return CGen.Posix.setresgid(rgid, egid, sgid);
    }

    public static long syscall(long nr, long a1, long a2, long a3, long a4, long a5) {
        return CGen.Posix.syscall(nr, a1, a2, a3, a4, a5);
    }

    // --- pointer / struct / string marshalling: Panama FFM ---
